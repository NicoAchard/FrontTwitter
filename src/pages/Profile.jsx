...
export default () => {
  ...
  //axios  buscar usuario con el username del  params
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const options = {
          method: "GET",
          url: `http://localhost:3000/usuarios/${params.username}`,
          headers: {
            Authorization: `Bearer ${token}`,
          },
        };
        const response = await axios.request(options);
        console.log(response.data.userUrl);
        setUser(response.data.userUrl);
      } catch (error) {
